...
4. Keys: the NAME goes in `.env.example` with a one-line comment; the VALUE goes in `.env` (gitignored) and the owner's password manager. Secret values never appear in tracked files or chat — that is Never-tier.
